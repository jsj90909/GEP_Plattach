...
{ // 블록에 관한 정보
    ...
    { // 블록 색상
        ...
        FIRST = PINK,
        LAST = ORANGE, // 초기 컬러(분홍색), 최종 컬러(주황색)
...
public class BlockControl : MonoBehaviour
{
    ...
    // 무지개 블록 여부
    public bool is_rainbow = false;

    // 블록 위에 표시할 무지개 반투명 패널
    // Unity Inspector에서 BlockPrefab의 자식 오브젝트를 연결해야 함
    public GameObject rainbow_panel = null;
    ...
    void Start()
    {
        ...
        // 처음 시작할 때 무지개 패널 상태를 현재 is_rainbow 값에 맞춤
        this.SetRainbow(this.is_rainbow);
    ...
    { // 블록이 잡혔을 때에 블록을 크게 하고 그렇지 않을 때는 원래대로 돌아감
        ...
        { // '다음 블록' 상태가 '정보 없음' 이외인 동안 -> '다음 블록' 상태가 변경된 경우
            ...
            switch (this.step)
            {
                ...
                case Block.STEP.GRABBED: // ＇잡힌＇ 상태
                    this.transform.localScale = Vector3.one * 1.2f; // 블록 표시 크기를 크게 한다
                    break;

                case Block.STEP.RELEASED: // '떨어져 있는' 상태
                    this.position_offset = Vector3.zero;
                    this.transform.localScale = Vector3.one * 1.0f; // 블록 표시 크기를 보통 사이즈로 함
                    break;
        ...
        {
            case Block.STEP.GRABBED: // 잡힌 상태
                this.slide_dir = this.calcSlideDir(mouse_position_xy); // 잡힌 상태일 때는 항상 슬라이드 방향을 체크
                break;
    ...
    {
        this.is_rainbow = enable;

        Debug.Log("[BlockControl] SetRainbow 호출됨 / " + this.name + " / enable: " + enable + " / is_rainbow: " + this.is_rainbow);

        if (this.rainbow_panel == null)
        {
            if (enable)
            {
                Debug.LogWarning("[BlockControl] Rainbow Panel이 연결되지 않았습니다: " + this.name);
            }

            return;
        }

        bool renderer_visible = this.GetComponent<Renderer>().enabled;
        this.rainbow_panel.SetActive(enable && renderer_visible);
    }
    ...
    { // 지정된 마우스 좌표가 자신과 겹치는지 반환
        ...
        do
        {
            ...
            {
                break;
            } // X 좌표가 자신과 겹치지 않으면 루프를 빠져 나감

            if (position.y < center.y - h || center.y + h < position.y)
            {
                break;
            } // Y 좌표가 자신과 겹치지 않으면 루프를 빠져 나감
    ...
    { // 연소 시간 값을 상수값에서 현시점의 레벨에서 정해진 값으로 수정
    ...
    public void setVisible(bool is_visible)
    {
        ...
        // 블록이 보일 때만 무지개 패널도 보이도록 처리
        // 블록이 VACANT가 되었는데 패널만 남는 문제 방지
        if (this.rainbow_panel != null)
        {
            this.rainbow_panel.SetActive(is_visible && this.is_rainbow);
        }
    }
    ...
    { // 낙하 시작 처리
        ...
        this.position_offset.y = (float)(start.i_pos.y - this.i_pos.y) * Block.COLLISION_SIZE; // 지정된 블록에서 좌표를 계산
    }

    public void beginRespawn(int start_ipos_y)
    { // 색이 바꿔 낙하 상태로 하고 지정한 위치에 재배치
        // 새로 생성되는 블록은 무지개 블록이 아니도록 초기화
        this.SetRainbow(false);
